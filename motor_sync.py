...
class sync:
            ...
            if len(pulses) > 0:
               while len(pulses) > 0:
                  if self._sbc.tx_room(
                     self._chip, self._leader, lgpio.TX_WAVE) > 0:
                     self._sbc.tx_wave(
            ...
            if self._busy:
               ...
               while True:
                  ...
                  if last_event_time is not None:
                     pulses.append(
                        lgpio.pulse(gbits, lgpio.GROUP_ALL,
               ...
               if last_event_time is not None:
                  pulses.append(lgpio.pulse(gbits, lgpio.GROUP_ALL, 0))
   ...
   def busy(self):
      ...
      return self._build.busy() or self._transmit.busy() or self._sbc.tx_busy(
         self._chip, self._group.leader(), lgpio.TX_WAVE)
